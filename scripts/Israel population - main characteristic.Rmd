---
title: "Israel population - main characteristic"
...
```
בתור התחלה נציג את התפלגות הגילים של כלל אוכלוסיית ישראל בשנה המעודכנת ביותר שיש לי - שנת 2019


```{r}
p_general_distribution <- population_2000_2019_int %>%
...
p_population_grouop <- population_2000_2019_int %>%
...
כאן אני רוצה להראות את התפלגות הגילים השונה של כל מגדר, ואת היחס בין גברים לנשים לפי גיל
בהתחלה עשיתי את זה עבור יהודים וערבים בנפרד אבל בדיעבד לא נראה לי שיש בזה טעם. 

```{r eval=FALSE, include=FALSE}
p_jews_gender<- population_2000_2019_int %>%
...
p_arabs_gender<- population_2000_2019_int %>%
...
plot_grid(jews_gender, arabs_gender)
...
```{r}
p_all_gender<- population_2000_2019_int %>%
...
all_gender_rate <- population_2000_2019_int %>%
  filter( population_group %in% c("all_male","all_female"),
          year == 2019) %>%
